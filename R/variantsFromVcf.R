...
#'
#' [Jesko:] Now returns with more information: reference and alternative chromosome and position, breakpoint homology.
#'
...
variantsFromVcf <- function(
  ...
  if(sv.caller=='gridss'){
    ...
    vcf <- cbind(vcf, getInfoValues(vcf$info, 'HOMLEN'))
    vcf$info <- NULL
    colnames(vcf) <- tolower(colnames(vcf))
    ...
    out <- with(df,{
      do.call(rbind,Map(function(partner_type, chrom_ref, chrom_alt, alt, sv_len_pre, pos_ref, pos_alt, homlen){
        if(partner_type == 'b'){
          sv_type <- 'SGL'
        ...
        return(data.frame(chrom_ref, pos_ref, chrom_alt, pos_alt, sv_type, sv_len, homlen, stringsAsFactors = F))
      }, partner_type, chrom_ref, chrom_alt, alt, sv_len_pre, pos_ref, pos_alt, homlen, USE.NAMES = F))
    })
